...
#######################################################
 ...
 function(input, output,session) {
  ...
  output$contents11 <- renderTable({
    req(input$filecsv)#bloque la suite du code si vide
    df1 <- read.csv(input$filecsv$datapath, header = TRUE, sep = ";")
    #df1<-read_delim(input$filecsv$datapath, delim = ";", escape_double = FALSE, trim_ws = TRUE)
    return(tail(df1, n = 1))
    })
  ...
  observeEvent(input$ValidTab, {
    ...
    if(!is.null(input$filecsv$datapath)){
      ...
      #data1$table<-read_delim(input$filecsv$datapath, delim = ";", escape_double = FALSE, trim_ws = TRUE)
  ...
  #previsualisation fichier Hobo
  output$contents3 <- renderTable({
    req(input$filetxt)
    df2 <- read.delim(input$filetxt$datapath, header = TRUE, sep = "\t")
    return(head(df2, n = 1))
  })
  
  output$contents33 <- renderTable({
    req(input$filetxt)
    df2 <- read.delim(input$filetxt$datapath, header = TRUE, sep = "\t")
    return(tail(df2, n = 1))
  })
  ...
  #lecture fichier Hobo
  observeEvent(input$ValidHobo, {
    
    if(!is.null(input$filetxt$datapath)){
      data2$table<- read.delim(input$filetxt$datapath, header = TRUE, sep = "\t")
      sendSweetAlert(session = session, title = "Done !", text = "Le fichier a bien ete lu !",type = "success")
    }
    #updateTabItems(session, "tabs", selected = "visualization")
    #}
...
#      mylist(list(contacts=data1$table,HT=data2$table,X=data3$DT[1,2],Y=data3$DT[1,3],x=data3$DT[1,4],y=data3$DT[1,5],
#                  obs=data3$DT[1,6],Chemin=data3$DT[1,1]))
#    }else{
#      mylist(list(contacts=data1$table,X=data3$DT[1,1],Y=data3$DT[1,2],x=data3$DT[1,4],y=data3$DT[1,5]
#                  ,obs=data3$DT[1,3],Chemin=data3$DT[1,1]))
#    }
#    })
  ...
  output$download <- downloadHandler(
    ...
    filename = function() {"participation.xlsx"},
